...
pub mod u128_dec_format_compatible {
        ...
        D: Deserializer<'de>,
    {
        match U128::deserialize(deserializer)? {
            U128::Number(value) => Ok(value),
            U128::String(value) => value.parse::<u128>().map_err(de::Error::custom),
        }
    }
}
...
pub mod u64_dec_format_compatible {
        ...
        D: Deserializer<'de>,
    {
        match U64::deserialize(deserializer)? {
            U64::Number(value) => Ok(value),
            U64::String(value) => value.parse::<u64>().map_err(de::Error::custom),
        }
    }
}
...
pub mod hashes {
    ...
    pub fn combine_hash2(x: Hash, y: Hash) -> Hash {
        ...
        env::sha256(&combined).try_into().unwrap()
    }
    pub fn combine_hash3(x: Hash, y: Hash, z: Hash) -> Hash {
        combine_hash2(combine_hash2(x, y), z)
    }
    pub fn combine_hash4(x: Hash, y: Hash, z: Hash, q: Hash) -> Hash {
        combine_hash2(combine_hash3(x, y, z), q)
    }

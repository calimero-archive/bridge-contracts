...
impl From<Secp256K1PublicKey> for PublicKey {
    fn from(libsecp256k1: Secp256K1PublicKey) -> Self {
        Self::SECP256K1(libsecp256k1)
    }
}
    ...
    pub fn sign(&self, data: &[u8]) -> Signature {
        match &self {
            ...
            SecretKey::SECP256K1(secret_key) => {
                let (signature, rec_id) = libsecp256k1::sign(
                    &libsecp256k1::Message::parse(data.try_into().unwrap()),
                    secret_key,
                );
                let data = (signature as libsecp256k1::Signature).serialize();
                let mut buf = [0; 65];
...
impl std::fmt::Display for SecretKey {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        let data = match self {
            ...
            SecretKey::SECP256K1(secret_key) => bs58::encode(secret_key.serialize()).into_string(),
        };
...
impl FromStr for SecretKey {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match key_type {
            ...
            KeyType::SECP256K1 => {
                let mut array = [0; libsecp256k1::util::SECRET_KEY_SIZE];
                let length = bs58::decode(key_data).into(&mut array[..]).map_err(|err| {
                    Self::Err::InvalidData {
                        error_message: err.to_string(),
                    }
                })?;
                if length != libsecp256k1::util::SECRET_KEY_SIZE {
                    return Err(Self::Err::InvalidLength {
                        expected_length: libsecp256k1::util::SECRET_KEY_SIZE,
                        received_length: length,
                    });
                }
                Ok(Self::SECP256K1(
                    libsecp256k1::SecretKey::parse(&array.try_into().unwrap()).map_err(|err| {
                        Self::Err::InvalidData {
                            error_message: err.to_string(),
                        }
                    })?,
...
impl Secp256K1Signature {
    ...
    pub fn recover(
        ...
        let rec_id = libsecp256k1::RecoveryId::parse(self.0[64]).unwrap();
        let signature = libsecp256k1::Signature::parse_standard(&self.0[0..64].try_into().unwrap())
            .map_err(|err| crate::errors::ParseSignatureError::InvalidData {
                error_message: err.to_string(),
            })?;
        let msg = Message::parse(&msg);

        let res = libsecp256k1::recover(&msg, &signature, &rec_id)
            .map_err(|err| crate::errors::ParseSignatureError::InvalidData {
                error_message: err.to_string(),
            })?
            .serialize();
    ...
    pub fn verify(&self, data: &[u8], public_key: &PublicKey) -> bool {
        match (&self, public_key) {
            ...
            (Signature::SECP256K1(signature), PublicKey::SECP256K1(public_key)) => {
                let _rec_id = libsecp256k1::RecoveryId::parse(signature.0[64]).unwrap();
                let sig =
                    libsecp256k1::Signature::parse_standard(&signature.0[0..64].try_into().unwrap())
                        .unwrap();
                let pdata: [u8; 65] = {
                ...
                libsecp256k1::verify(
                    &libsecp256k1::Message::parse(data.try_into().unwrap()),
                    &sig,
                    &libsecp256k1::PublicKey::parse(&pdata).unwrap(),
                )
            }
            _ => false,
        }
    ...
    pub fn empty(key_type: KeyType) -> Self {
        match key_type {
            KeyType::ED25519 => Signature::ED25519(
                ed25519_dalek::Signature::from_bytes(&[0u8; ed25519_dalek::SIGNATURE_LENGTH])
                    .unwrap(),
            ),
            _ => unimplemented!(),
        }
